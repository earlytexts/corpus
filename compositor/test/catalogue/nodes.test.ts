 ...
 * (../../src/core/catalogue/nodes.ts): the node→file-path lookups, author
 * indexing, the document→edition map and borrowed-child traversal, and the
 * label helpers. editionPath is exercised through the replace-scope planner;
 * authorPath is only reached from the VSCode surface, so it is pinned here
 * directly, over a real catalogue author. The rest are unit-tested over
 * structural fixtures — the logic only cares about object identity and a few
 * fields — in the spirit of catalogueWalk.test.ts.
 */
...
import {
  ...
  type TreeNode,
  workDocId,
} from "../../src/core/catalogue/nodes.ts";
