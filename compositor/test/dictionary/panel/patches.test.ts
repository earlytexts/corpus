/**
 * The pure patches behind the panel's optimistic updates
 * (core/dictionary/panel/patches.ts): swapping one shard's entries into a
 * cached dictionary, and dropping the curation rows a written entry accounts
 * for. Both must match what a full re-read/re-rank would produce for the
 * surfaces they touch, and leave everything else alone.
 */
...
import {
  dropCuratedRows,
  replaceShardEntries,
} from "../../../src/core/dictionary/panel/patches.ts";
import type { CurationRow } from "../../../src/core/dictionary/curation.ts";
