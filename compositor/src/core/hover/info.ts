 ...
 * token-accounting hover. Given a printed word, the register, and the
 * enclosing text's `[metadata.dictionary]` overrides, it reports the citation
 * lemma the token resolves to, the normalised form it takes within that
 * lemma's paradigm, and (for an ambiguous surface) the other lemmas on offer,
 * each with the `[w:]` value that would pin it.
 *
 ...
 * (adapters/vscode/hover.ts) renders this and wires the pin action.
 */
