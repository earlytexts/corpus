 ...
 * a scanner suggestion to the markup that would wrap it. `scanSource`
 * (hints.ts) finds the candidates (people, places, organisations, citations,
 * foreign text) in an edition's source; this decides what each "mark this up"
 * quick fix writes, and the wording it carries. Kept apart from the vscode
 * wiring (adapters/vscode/commands/suggestMarkup.ts) so the rules are
 * unit-testable without the editor API. The overlay is all-or-nothing (no
 * per-category filter), so `suggestionKey` exists only to group repeated
 * identical matches for the "mark up all N" fix.
 */
