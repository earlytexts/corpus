 ...
 * accumulates. Pure logic, editor-free
 * (adapters/vscode/commands/suggestMarkup.ts wires it to VSCode); it reads the
 * compiled catalogue and the wire types the corpus owns, but the read-side
 * text processing lives here, not in the corpus.
 *
 ...
 * evidence on their own; common ones are WEAK and only ever match as part of a
 * cluster anchored by a strong word. A small per-language override table
 ...
 *  - scanSource: place the file's source tokens (shared/sourceTokens.ts runs
 *    Markit's own tokenizer over a positioned compile, so word identity is
 *    Markit's — the same `~` joins, page-break and editorial transparency the
 *    whole pipeline shares — and drops the tokens already inside markup),
 *    match them and the markup-masked lines against the lexicons, and report
...
import { distinctEditionDocuments, distinctWorks } from "../catalogue/walk.ts";
import {
  blockTokens,
  collectBlocks,
  FORMAT_DELIMS,
