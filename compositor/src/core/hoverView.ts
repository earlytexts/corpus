 ...
 * surface/hover.ts. Given a `.mit` document's text and a cursor position, it
 * locates the token under the cursor (reusing the source-token walk the squiggle
 * and suggestion overlays share, so exempt and already-`[w:]`-marked tokens are
 * transparently skipped), asks the register how it accounts for that token
 * (`resolveHoverInfo`/`lemmaForms`, hoverInfo.ts), and renders the result as a
 * Markdown *string* plus the range it covers.
 *
 ...
 * "not in the dictionary" squiggle is the one tooltip it shows), so every hover
 * a contributor sees reads as the same lemma view rather than betraying which
 * route accounted for the word.
 *
 * Compiling the source to place tokens is memoised per (uri, version) and the
 * register's paradigms are indexed once per catalogue — both memos live here so
 * a burst of hovers over one unchanged document does the work once. The editor
 * layer (surface/hover.ts) wraps the returned Markdown in a scoped-trusted
 * `MarkdownString`, maps the range/position, and applies the pin edit.
 */
