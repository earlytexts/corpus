/**
 * The scaffolding flows: New Author, New Work (with its first edition), and New
 * Edition. Each gathers the required metadata, writes the files in the canonical
 * shape (templates.ts), and reveals the result. The vscode-free heart lives
 * here — the flow decisions (which author, which files, whether to make the new
 * edition canonical), the field validators, and the canonical rewrite — written
 * over two ports: the corpus filesystem it writes through, an `open` to reveal a
 * file, and a `ScaffoldPrompts` that gathers each item's metadata.
 *
 * The editor layer (commands/scaffolds.ts) implements the prompts as input
 * boxes (wiring in the validators exported here) and provides the fs/open
 * adapters; the corpus watcher picks the new files up, so the tree and
 * diagnostics refresh on their own.
 */

import type { Author, CorpusFsWrite, Work } from "@earlytexts/corpus";
import { YEAR } from "@earlytexts/corpus";
import { capitalize, workDocId } from "./nodes.ts";
