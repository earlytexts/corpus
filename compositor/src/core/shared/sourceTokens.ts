 ...
 * printed display, and the 0-based, end-exclusive line/column range it
 * occupies — with markup-exempt tokens (people, places, orgs, citations,
 * foreign spans, `[w:]`) dropped and `{…}`/`{{…}}` character/Greek-mode spans
 * absorbed so a replacement over a token covers the whole brace span. Pure,
 * editor-free.
 *
 ...
 * (dictionary/scan.ts) locates unaccounted surfaces by them. This module also
 ...
 * range), and `foldWord` (the lexicon-folding atom the mined lexicons and
 * these tokens must agree on).
 *
 ...
 * the dictionary scan (dictionary/scan.ts) sees the whole stream (it re-folds
