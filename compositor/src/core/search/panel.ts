/**
 * The pure core of the search panel: build a matcher from the query (literal
 * or regex, case toggle, whole-word wrapping), decide which lines of an
 * edition are block content (never the title line, `[metadata]` sections, or
 * `{#…}` tags), scope the catalogue's editions by author include/exclude
 * filters, scan one file's searchable lines into positioned matches with
 * clipped previews, and expand a replacement string for one match (regex
 * captures included). No VSCode — the provider
 * (adapters/vscode/searchPanel.ts) runs these over the model's compiled files
 * and applies the edits; everything decidable without an editor is decided
 * here, and tested.
 *
...
import { distinctWorks } from "../catalogue/walk.ts";
import { collectBlocks } from "../shared/sourceTokens.ts";
