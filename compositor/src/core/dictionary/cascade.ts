 ...
 * — so the register is never left invalid. A respelling names a target
 * spelling, a lemma its citation form; if a target is itself unregistered it
 * is added (or refused) first, recursing until everything bottoms out in the
 * register.
 *
 * The branch logic here is vscode-free and unit-tested; the editor layer
 * (adapters/vscode/commands/dictionaryDiagnostics.ts) injects the prompts the
 * cascade drives and writes the resulting decisions across their shards. The
 * resolution *rules* — attested vs. unattested, which kinds keep the register
 * valid — live in dictionary/resolve.ts; this is the interactive walk over
 * them.
 */

import { type EntryValue, shardOf } from "@earlytexts/corpus";
import type { EntryAction } from "./edits.ts";
import { unattestedRejectMessage } from "./entryText.ts";
import { resolveLemmaTarget, resolveSpellingTarget } from "./resolve.ts";
