 ...
 * this module turns it into the `data` message and keeps the `PanelCache` a
 * full refresh, an optimistic patch, and a re-read produce. The two data
 * patches it composes (`replaceShardEntries`, `dropCuratedRows`) live in
 * panel/patches.ts; the two views (`dictionaryViews`) and the ranking
...
import { dictionaryViews, type LemmaRow, type VariantRow } from "../views.ts";
import {
  type CurationRow,
  curationRows,
  type TokenIndex,
} from "../curation.ts";
import { dropCuratedRows, replaceShardEntries } from "./patches.ts";
