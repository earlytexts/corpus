/**
 * The two cross-cut views the dictionary panel browses, derived purely from
 * the authored (raw) dictionary. The shards are keyed by *surface*, so neither
 * view the curator wants is local: a variant-spelling map is the scattered
 * handful of cross-reference entries, and a lemma's forms are its headword
 * plus every inflection pointing back at it (often alphabetically adjacent,
 * but scattered for irregulars — went/go). Both are reconstructed here in one
 * pass so the panel can filter by letter and paginate over them; editing still
 * writes single surface entries back through `dictionary/edits.ts`.
 */
