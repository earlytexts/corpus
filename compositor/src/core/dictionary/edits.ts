 ...
 * The editor layer (adapters/vscode/commands/dictionaryDiagnostics.ts) reads
 * the current shard, calls one of these, and writes the result back;
 * canonicalisation (sorting, one entry per line, minimal micro-syntax) is the
 * corpus's own `shardDictionary`, so an entry added from the editor is
 * byte-identical to one `deno task fmt` would produce — it round-trips through
 * corpus validation.
 *
