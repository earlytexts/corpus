/**
 * The resolution rule behind adding a dictionary entry from the editor: a
 * value (a respelling target or a lemma) must bottom out in the register, so
 * before an entry is written every target it names is resolved — already
 * registered, added alongside it, or refused. This encodes the corpus's
 * attestation rule on the write side (see the corpus DICTIONARY.md): a
 * respelling must point to a spelling that appears in the texts, so an
 * unattested spelling target is rejected; a lemma is a citation form and may
 * be unprinted, so an unattested lemma is allowed on explicit confirmation.
 * Pure and vitest-tested; the editor layer
 * (adapters/vscode/commands/dictionaryDiagnostics.ts) drives the prompts
 * around these decisions and writes the resulting entries across their shards.
 */

import { accountTokens, type Catalogue } from "@earlytexts/corpus";
import { distinctEditionDocuments } from "../catalogue/walk.ts";
