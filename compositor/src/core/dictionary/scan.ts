 ...
 * for a compiled document, which folded surfaces are unaccounted (no
 * dictionary entry). This module only *locates* those surfaces in the source,
 * so the editor can squiggle each occurrence and offer curation quick-fixes.
 *
 * Location runs Markit's own tokenizer over the positioned compile
 * (shared/sourceTokens.ts's `blockSourceTokens`), which drops exempting markup
 * (people, places, orgs, citations, foreign spans, `[w:]`) and reads through
 * page breaks, editorial marks, and `{…}` character mode — so an exempt or
 ...
 * `~`-fused run with no dictionary entry reads as one unregistered token here,
 * two accounted ones there) — simply goes unflagged rather than mis-flagged;
 * the coverage *counts* (which need no ranges) stay exact.
 *
