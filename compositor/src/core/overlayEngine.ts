 ...
 * (commands/dictionaryDiagnostics.ts and commands/suggestMarkup.ts) — the
 * vscode-free core of it. Both scan the open editions on demand and squiggle
 * their findings, gated behind a boolean setting and re-run on the same four
 * triggers: the setting flips, the active editor changes, an edit lands
 * (debounced), and the corpus reloads.
 *
 * This engine owns every decision of that machinery — the scanned-map state, the
 * per-document debounce, and the scan/drop/refresh branching — written as an
 * orchestrator over ports that speak the overlay's own vocabulary (`enabled`,
 * `prepare`, `scan`, `render`), never vscode's. A document is opaque (`Doc`):
 * the engine only ever asks a port for its path, so the diagnostic collection,
 * the config gate, the compile-and-scan, and the workspace subscriptions all
 * stay in the adapter, which feeds this the events and executes the effects.
 */
