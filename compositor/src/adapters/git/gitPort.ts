/**
 * The one place isomorphic-git lives: the `GitPort` implementation the core
 * declares. Cloning and remote-config serve the onboarding flow (setup.ts); the
 * rest — what changed, branch, commit, merge, push — serve the contribution
 * flow (workflow.ts). Keeping git behind the narrow port is what lets the
 * workflow logic be reasoned about and tested without a real repository, and
 * means the extension bundles its own git (no system install required of
 * contributors).
 *
