...
- `@earlytexts/corpus/test` (`src/test.ts`) — the in-memory corpus builder the corpus's and the computer's tests share, plus `buildCatalogue` (the compiler) to compile a fixture map into a catalogue in memory. This is the computer's only door onto the compiler: in production it builds `catalogue/` by running this checkout's own `deno task build` (its `scripts/build-corpus.ts` shells out to it), not by importing the compiler.
